...
if (!is_admin() ) {
	...
	wp_enqueue_script('modernizr', get_template_directory_uri() . '/js/modernizr-2.5.0.min.js', array('jquery'));

	//ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js
	/*
	wp_enqueue_script('general', get_template_directory_uri() . '/wp-content/themes/gcc/scripts/general.js', array('jquery'));


	wp_register_script('general', get_bloginfo('template_directory') . '/scripts/general.js');
...
function remove_dashboard_widgets(){
  ...
  unset($wp_meta_boxes['dashboard']['side']['core']['dashboard_tribe_dashboard_widget']);
}
...
function html5_search_form( $form ) {
    ...
    <input type="search" placeholder="'.__("").'" value="' . get_search_query() . '" name="s" id="s" class="rnd5" style="border:solid 1px #ccc; padding:5px 10px;" />
...
function register_replacement( $link ){
	if ( ! is_user_logged_in() ) {
		...
		$link = $before . '<a href="' . admin_url() . '">' . __('Admin') . '</a>' . $after;
	}
	return $link;
}
...
/*wp_enqueue_script('iso_script', get_template_directory_uri() . '/js/jquery.isotope.min.js', array('jquery'));
wp_enqueue_script('my_script', get_template_directory_uri() . '/js/myScript.js', array('jQuery', 'modernizr'));
...
// Change login image -----------------------------------------------------
    ...
    h1 a { background-image:url('.get_bloginfo('template_directory').'/images/xxxx.png) !important; }
    </style>';
    }
    add_action('login_head', 'my_custom_login_logo');

    function put_my_url(){
    return ('http://michaelcockle.co.uk/');
    }
...
function my_register_image_sizes() {
	if ( function_exists( 'add_image_size' ) ) {
		add_image_size( 'fiveseventy', 570, 570, false );
	}
}
...
// Show content on HOME page ps you can even show pages 'page' ------------------------------------
...
function my_get_posts( $query ) {
	...
	if ( is_home() && false == $query->query_vars['suppress_filters'] )
		$query->set( 'post_type', array( 'post', 'videos', 'reviews', 'quote', 'attachment' ) );

	return $query;
}
*/

// Include custom posts in specific author pages ----------------------------------------------


/*function add_pagination_to_author_page_query_string($query_string)
{
    if (isset($query_string['author_name'])) $query_string['post_type'] = array('post','reviews');
    return $query_string;
...
// https://github.com/retlehs/roots/issues/33
